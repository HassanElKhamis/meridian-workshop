...
export default {
  ...
  setup() {
    ...
    const calculateSummaryStats = () => {
      totalRevenue.value = monthlyData.value.reduce((sum, m) => sum + m.revenue, 0)
      avgMonthlyRevenue.value = monthlyData.value.length
        ? totalRevenue.value / monthlyData.value.length
        : 0
      totalOrders.value = monthlyData.value.reduce((sum, m) => sum + m.order_count, 0)

      const best = quarterlyData.value.reduce((a, b) =>
        a.total_revenue > b.total_revenue ? a : b, quarterlyData.value[0])
      bestQuarter.value = best ? best.quarter : ''
    }
    ...
    const getBarHeight = (revenue) => {
      var maxRevenue = Math.max(...monthlyData.value.map(m => m.revenue), 0)
